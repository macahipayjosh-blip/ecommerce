...
function Countdown({ endTime }: { endTime: string }) {
    ...
    return <span style={{ fontFamily: 'monospace', fontWeight: 700, color: '#a32d2d' }}>{h}:{m}:{s}</span>;
}
...
export default function SellerAuctionShow({ auction }: Props) {
    ...
    const avgBid = auction.bids.length
        ? auction.bids.reduce((sum, b) => sum + parseFloat(b.amount), 0) / auction.bids.length
        : 0;
        ...
        <SellerLayout breadcrumb={auction.name}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    <Link href={route('seller.auctions.edit', auction.id)} style={{ padding: '8px 14px', border: '1px solid #e8e8e4', fontSize: 13, color: '#333', textDecoration: 'none' }}>
                        Edit
                    </Link>
                    <span style={{ display: 'inline-flex', alignItems: 'center', padding: '8px 14px', borderRadius: 4, fontSize: 12, fontWeight: 700, ...sc }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 20 }}>
                {[
                    { icon: <TrendingUp size={18} color="#2d6a2d" />, label: 'Highest Bid', value: highestBid ? `₱${parseFloat(highestBid.amount).toFixed(2)}` : '—', highlight: true },
                    ...
                    { icon: <Clock size={18} color="#a32d2d" />, label: isActive ? 'Time Left' : 'Ended', value: isActive ? <Countdown endTime={auction.auction_end_at} /> : new Date(auction.auction_end_at).toLocaleDateString() },
                ].map((s, i) => (
                    <div key={i} style={{ border: '1px solid #e8e8e4', background: '#fff', padding: '16px 18px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                            {s.icon}
                            <span style={{ fontSize: 11, color: '#999', fontFamily: "'DM Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.06em' }}>{s.label}</span>
                        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div style={{ border: '1px solid #e8e8e4', background: '#fff', overflow: 'hidden' }}>
                        <div style={{ height: 220, overflow: 'hidden', background: '#f5f5f3', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {primaryImg ? (
                                <img src={`/storage/${primaryImg.image_path}`} alt={auction.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            ) : (
                        ...
                        {auction.images.length > 1 && (
                            <div style={{ display: 'flex', gap: 6, padding: 10, overflowX: 'auto' }}>
                                {auction.images.map((img) => (
                                    <img key={img.id} src={`/storage/${img.image_path}`} alt="" style={{ width: 48, height: 48, objectFit: 'cover', border: img.is_primary ? '2px solid #0d0d0d' : '1px solid #e8e8e4', flexShrink: 0 }} />
                                ))}
                            </div>
                        )}
                    </div>
                    ...
                    <div style={{ border: '1px solid #e8e8e4', background: '#fff' }}>
                        <div style={{ padding: '12px 16px', borderBottom: '1px solid #e8e8e4', fontSize: 11, fontFamily: "'DM Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.08em', color: '#666' }}>Details</div>
                ...
                {/* Right — bid history */}
                <div style={{ border: '1px solid #e8e8e4', background: '#fff' }}>
                    <div style={{ padding: '14px 18px', borderBottom: '1px solid #e8e8e4', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", textTransform: 'uppercase', letterSpacing: '0.08em', color: '#666' }}>
                            Bid History
                        </span>
                        <span style={{ fontSize: 12, color: '#999' }}>{auction.bids.length} bid{auction.bids.length !== 1 ? 's' : ''}</span>
                    </div>
                                ...
                                <thead>
                                    <tr style={{ background: '#fafaf8', borderBottom: '1px solid #e8e8e4' }}>
                                        {['#', 'Bidder', 'Email', 'Amount', 'Time'].map((h) => (
                                            <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: '#999', fontFamily: "'DM Mono', monospace", textTransform: 'uppercase' }}>{h}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {auction.bids.map((bid, idx) => (
                                        <tr key={bid.id} style={{ borderBottom: '1px solid #f0f0f0', background: idx === 0 ? '#f0faf0' : 'transparent' }}>
                                            <td style={{ padding: '10px 14px', fontSize: 12, color: '#999' }}>#{auction.bids.length - idx}</td>
                                            <td style={{ padding: '10px 14px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: idx === 0 ? '#2d6a2d' : '#e8e8e4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: idx === 0 ? '#fff' : '#666', flexShrink: 0 }}>
                                            ...
                                            <td style={{ padding: '10px 14px', fontSize: 14, fontWeight: 700, color: idx === 0 ? '#2d6a2d' : '#0d0d0d' }}>
                                                ₱{parseFloat(bid.amount).toFixed(2)}
                                            </td>
                                            <td style={{ padding: '10px 14px', fontSize: 12, color: '#999' }}>{new Date(bid.created_at).toLocaleString()}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
